(() => {
  ...
  fetchJson('../data/canonical/G-S02.json').then(snapshot=>{const target={...snapshot,members:snapshot.member_count,date_persian:snapshot.official_timestamp_persian.split(' ')[0],time_iran:snapshot.official_timestamp_persian.split(' ')[1],type:'period'};const canonical=viewerData.buildMembers(snapshot);const metrics={baseline_snapshot_id:snapshot.snapshot_id,league_week:'G-S02',period_clan_medals_change:1451196,period_kills_change:128098,weekly_clan_medals_earned:1451196,weekly_kills_earned:128098};render(target,metrics,canonical.members,requestedMode,'canonical',[target]);}).catch(error=>{console.error(error);root.innerHTML='<p class="error">دادهٔ Snapshot رسمی GOLDENCROWN قابل بارگذاری نیست.</p>';});
  ...
  function render(target, metrics, members, mode, dataMode, snapshots) {
    ...
    simpleButton.onclick = () => { currentMode = 'simple'; renderResults(); };
