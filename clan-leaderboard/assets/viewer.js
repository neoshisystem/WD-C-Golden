(() => {
  ...
  fetchJson('../data/canonical/G-S01.json').then(snapshot=>{const target={...snapshot,members:snapshot.member_count,date_persian:snapshot.official_timestamp_persian.split(' ')[0],time_iran:snapshot.official_timestamp_persian.split(' ')[1],type:'baseline'};const canonical=viewerData.buildMembers(snapshot);const metrics={baseline_snapshot_id:snapshot.snapshot_id,league_week:'G-S01',period_clan_medals_change:0,period_kills_change:0,weekly_clan_medals_earned:0,weekly_kills_earned:0};render(target,metrics,canonical.members,requestedMode,'canonical',[target]);}).catch(error=>{console.error(error);root.innerHTML='<p class="error">دادهٔ Snapshot رسمی GOLDENCROWN قابل بارگذاری نیست.</p>';});
